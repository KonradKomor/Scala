...
class UserRegistrationService(private val userRepository: UserRepository, private val passwordValidationService: PasswordValidationService) {
  ...
  def registerUser(firstName: String, lastName: String, address: String, emailAddress: String, password: String): Boolean = {
    ...
    this.userRepository.save(new User(0, firstName, lastName, address, emailAddress, password))
